...
class Y2022D17c(private val jet: String) : Day {
    override fun part1(): Long = solve(2022)

    override fun part2(): Long = solve(1_000_000_000_000)
    ...
    private fun solve(n: Long): Long {
        ...
        for (i in 0 until n) {
            ...
            seen.put(Triple(rockIndex, jetIndex, state.rows.asList()), i)?.let { j ->
                val q = (n - j) / (i - j)
                val r = (n - j) % (i - j)
                return heights[(j + r).toInt()] + q * (state.height - heights[j.toInt()])
            }
...
fun main() = Day.runDay(Y2022D17c::class)