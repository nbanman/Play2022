...
class Y2023D1(input: String) : Day {
    ...
    override fun part2(): Int {
        ...
        val numbers = "one|two|three|four|five|six|seven|eight|nine".split('|')
        numbers
    ...
    private fun MutableMap<Char, Node>.populateTree(
        ...
        val firstChar = iterator.nextChar()
        var lastNode = getOrPut(firstChar) { Node(value) }
        while (iterator.hasNext()) {
            val nextChar = iterator.nextChar()
            lastNode = lastNode.children.getOrPut(nextChar) { Node(value) }
        }
    }
}
